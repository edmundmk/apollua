	...
	:	LuaValue
{
	...
	protected internal override bool TryToInteger( out int v )
	{
		v = value;
		return true;
	}
	...
	protected internal override LuaValue IntegerDivide( LuaValue o )
	{
		...
		if ( o.TryToDouble( out v ) )
		{
			return Math.Floor( (double)value / v );
		}
	...
	protected internal override LuaValue RaiseToPower( LuaValue o )
	{
		...
		if ( o.TryToInteger( out i ) )
		{
			if ( i >= 0 )
			{
				return MathEx.Pow( value, i );
			}
			else
			{
				return Math.Pow( (double)value, (double)i );
			}
		}
		double v;
		if ( o.TryToDouble( out v ) )
		{
			return Math.Pow( (double)value, v );
		}
